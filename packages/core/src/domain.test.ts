...
import {
  ...
  stateIntegrity, tryResourcePath, tokenHash, tokenId, type Binding, type BindingConfig, type CreateGrantInput,
...
const cap = (
  id: string,
  over: Partial<BindingConfig> = {},
): Binding => ({
  id: r(id),
  permissions: ['invoke'],
  ...over,
});
const storedCap = (id: string): Binding => cap(id, { permissions: ['read', 'invoke'] });
...
describe('creating a grant', () => {
  ...
  it('requires binding targets to be live and every binding to carry a permission', () => {
    ...
    expect(() => createGrant(fixture(), { ...input, bindings: [entry] }, g('child'), at))
      .toThrow('Binding target does not exist.');
    expect(() => createGrant(fixture(), {
      ...input, bindings: [cap('search-files', { permissions: [] })],
    }, g('child'), at)).toThrow('Select at least one permission.');
  });

  it('requires exactly one resource ID binding', () => {
    expect(() => createGrant(fixture(), {
      ...input, parentId: null,
      bindings: [{ path: 'acme/future', permissions: ['invoke'] } as unknown as Binding],
    }, g('path'), at)).toThrow('Binding must name one resource id.');
    expect(() => createGrant(fixture(), {
      ...input, parentId: null,
      bindings: [{ id: r('search-files'), path: 'acme/drive', permissions: ['invoke'] } as unknown as Binding],
    }, g('both'), at)).toThrow('Binding must name one resource id.');
  });
...
describe('setting bindings', () => {
  ...
  it('requires each binding to name a live resource', () => {
...
describe('identity brands', () => {
  it('keeps resource, grant, and token identities from being assigned to one another', () => {
    ...
    expectTypeOf<Binding>().toEqualTypeOf<BindingConfig & { id: ResourceId }>();
  });
});
