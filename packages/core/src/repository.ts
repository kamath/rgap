...
export type GrantWrite = { name: string; bindings: Binding[]; expiresAt: string | null };
...
export type BindingsHandle = Binding[] & {
  set(bindings: Binding[]): Promise<GrantHandle>;
};
...
export interface RgapCommands {
  ...
  setBindings(grantId: GrantId, bindings: Binding[]): Promise<Grant>;
...
function asGrant(commands: RgapCommands, grant: Grant): GrantHandle {
  const bindings = Object.assign([...grant.bindings], {
    set: async (entries: Binding[]) => asGrant(commands, await commands.setBindings(grant.id, entries)),
  });
  return {
    ...grant,
    bindings,
